 ...
 * 댓글 시트 흐름의 상태 전부.
 *
 * 의견 목록은 화면에 늘 붙어 있는 시트(PeekSheet)가 맡고, 여기서 다루는 것은
 * 그 위로 겹쳐 올라오는 댓글 시트뿐이다. 댓글 시트는 보고 있는 대목(passageId)과
 * 가림막(isMasked)에 매여 있다 — 대목이 바뀌거나 가림막이 다시 씌워지면 닫혀야 한다.
 * 딥링크가 지목한 흔적을 여는 것도 같은 규칙 아래 있어야 해서 함께 둔다.
 */
export type CommentSheetModel = {
  /** 댓글 시트가 올라와 있는 의견 — null이면 닫힘 */
  commentOpinionId: number | null
  /**
   * 딥링크로 이미 열어 준 의견. 한 번만 여는 데 쓴다 —
   * 기억하지 않으면 사용자가 다른 의견의 댓글로 옮겨가도 딥링크가 매 렌더 다시 끌어온다.
   */
...
export type CommentSheetAction =
  ...
  | { type: 'openComments'; opinionId: number }
  | { type: 'closeComments' }
