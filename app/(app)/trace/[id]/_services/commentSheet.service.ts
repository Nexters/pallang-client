...
import type { CommentSheetAction, CommentSheetModel } from '../_types/commentSheet.type'

export function createCommentSheetModel(input: {
 ...
 */
export function commentSheetReducer(
  state: CommentSheetModel,
  action: CommentSheetAction,
): CommentSheetModel {
  // 가려진 의견은 댓글로도 읽을 수 없어야 한다. 블러는 그림이고 inert는 브라우저에만 있는
  // 방어라, 규칙은 여기 한 곳에서 집행한다(#49). 닫는 길까지 막으면 갇히므로 여는 쪽만 막는다.
  if (state.isMasked && action.type === 'openComments') return state

  switch (action.type) {
    case 'sync': {
      // 대목이 바뀌면 목록이 통째로 갈리므로 댓글 시트도 함께 닫는다 — 남겨두면 화면에 보이지도 않는
      // 이전 대목의 의견에 댓글이 등록된다(#128).
      const isPassageChanged = state.passageId !== action.passageId
      // 가림막이 다시 씌워지면(같은 페이지 탭을 다시 눌러 해제가 풀리는 경우) passageId는 그대로라
      // 위의 리셋에 걸리지 않는다. 시트가 남으면 더는 읽을 수 없는 의견에 댓글을 쓸 수 있다.
      const isMaskReapplied = action.isMasked && state.commentOpinionId !== null
      ...
      const next: CommentSheetModel = {
        commentOpinionId: shouldOpenDeepLink
          ? deepLink
          : shouldReset
            ? null
            : state.commentOpinionId,
      ...
      const isUnchanged =
        next.commentOpinionId === state.commentOpinionId &&
    ...
    case 'openComments':
      if (state.commentOpinionId === action.opinionId) return state
      return { ...state, commentOpinionId: action.opinionId }
    case 'closeComments':
      if (state.commentOpinionId === null) return state
      return { ...state, commentOpinionId: null }
  }
}
