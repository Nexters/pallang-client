...
type SourceOptionProps = {
  description: string
  /** 시안이 강조해 둔 카드(사진으로 입력)는 어두운 배경 + 흰 글씨로 그린다. */
  emphasized?: boolean
...
function SourceOption({
  ...
  return (
    // 시안 3077:16085 — 두 카드는 같은 모양이 아니다. '사진으로 입력'은 어두운 배경에 흰
    // 글씨와 오렌지 아이콘으로 강조되고, '직접 입력'만 밝은 카드로 남는다.
    // 밝은 카드는 누르는 동안 강조된 카드와 같은 모습으로 뒤집힌다(시안 2135:3707·2137:3764).
    // 이미 어두운 강조 카드는 뒤집을 곳이 없어 press의 눌림만으로 피드백을 준다.
    <button
      type="button"
      onClick={onClick}
      className={cn(
