...
// 동작과 그 이유는 네이티브 쪽 KakaoLoginPlugin.swift의 주석을 참고할 것.
...
// 네이티브 플러그인이 있는 환경인지 — 카카오톡 앱 전환은 iOS 앱에서만 붙어 있다.
// 클릭 시점에 물으면 되는 값이라 렌더 상태로 올리지 않는다(안드로이드 추가 시 여기만 넓히면 된다).
export function canUseNativeKakaoLogin(): boolean {
