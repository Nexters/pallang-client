...
export function markWithdrawalCompleted(): void {
  setSessionStorageItem(WITHDRAWAL_COMPLETED_KEY, '1')
}

/** 플래그를 지우지 않고 읽는다 — AuthProvider의 세션 만료 리다이렉트 억제용. */
export function hasPendingWithdrawalNotice(): boolean {
  return getSessionStorageItem(WITHDRAWAL_COMPLETED_KEY) === '1'
}

/** 마이페이지 도착 시 1회 소비한다. true면 탈퇴 완료 스낵바를 띄운다. */
export function consumeWithdrawalNotice(): boolean {
  if (getSessionStorageItem(WITHDRAWAL_COMPLETED_KEY) !== '1') return false
  removeSessionStorageItem(WITHDRAWAL_COMPLETED_KEY)
  return true
}
