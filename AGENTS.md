...
- **시트를 끄는 제스처는 `_hooks/useSheetDrag`(@use-gesture) 하나를 거치고, 손잡이(`SheetHandle`)가 있는 시트만 끌린다.** 손잡이가 "끌 수 있다"는 신호라 신호 없는 시트(폼·선택지)는 정적 모달로 남아 실수 닫힘이 없다 — `BottomSheet`는 `showHandle`로 켠다. 반환 props를 시트 요소에 스프레드한다(ref가 아니다 — 포털 팝업은 effect 시점에 ref가 비어 리스너가 안 붙는다). 손잡이·맨 위 목록의 아래 드래그·더 올라갈 수 있을 때의 위 드래그만 시트가 가져가고 나머지는 스크롤에 양보한다. 닫히기만 하는 시트는 `useSheetDragDismiss`, 두 높이 시트는 `useTraceSheet`처럼 그 위에 스냅 규칙만 얹는다. 안쪽 버튼 탭은 건드리지 않는다(3px 미만은 탭, 끌고 난 뒤 click은 차단).
...
- **(Android)** APK 빌드에 **JDK 21**과 **Android SDK**가 둘 다 필요하다. JDK만 있으면 `SDK location not found`로 멈춘다 — 셋업은 [docs/capacitor.md](docs/capacitor.md)의 "최초 1회 셋업 (SDK)"을 따른다. dev(http) 로드는 debug 매니페스트의 `usesCleartextTraffic`로 허용.
